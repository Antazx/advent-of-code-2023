...
export class GameRecord {
  ...
  getMaxSet(): Set {
    ...
    for (const set of this.game) {
      const green = set.green;
      const blue = set.blue;
      const red = set.red;

      if (green > maxSet.green) maxSet.green = green;
      if (red > maxSet.red) maxSet.red = red;
      if (blue > maxSet.blue) maxSet.blue = blue;
    }
