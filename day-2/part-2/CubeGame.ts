...
export class CubeGame {
  ...
  getSumOfPowerGames(): number {
    const maxSets = this.gameHistory.map((game) => game.getMaxSet());
    const powMaxSets = maxSets.map((set) => set.red * set.green * set.blue);
    return powMaxSets.reduce((acc, current) => acc + current, 0);
  }
}
